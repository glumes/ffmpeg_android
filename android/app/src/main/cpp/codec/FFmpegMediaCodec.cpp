...
int FFmpegMediaCodec::decode_write(AVCodecContext *avctx, AVPacket *packet) {
...
int FFmpegMediaCodec::decode(const std::string &input, jobject surface) {
    AVFormatContext *input_ctx = NULL;
    int video_stream, ret;
    AVStream *video = NULL;
    AVCodecContext *decoder_ctx = NULL;
    ...
    while (ret >= 0 || ret == -11) {
        ...
        if (video_stream == packet.stream_index)
            ret = decode_render(decoder_ctx, &packet);
...
int FFmpegMediaCodec::decode_render(AVCodecContext *avctx, AVPacket *packet) {
    AVFrame *frame = NULL, *sw_frame = NULL;
    AVFrame *tmp_frame = NULL;
    uint8_t *buffer = NULL;
    int size;
